...
class AccidentDetector:
    def __init__(
        self,
        model_path,
        frame_confirmation_threshold=3,
        accident_report_dir="accident_reports",
        output_video_filename="results_video.mp4",
        acc_conf_thresh=0.70,
        other_conf_thresh=0.60,
        speed_threshold=10,
        iou_threshold=0.1,
        location=None,
    ):
    ...
    def process_video(self, video_source):
        ...
        try:
            ...
            while cap.isOpened():
                ...
                if (
                    len(self.consecutive_accident_details)
                    >= self.frame_confirmation_threshold
    ...
    def _log_confirmed_event(self):
        self.logger.info(
            f"Confirmed accident event detected after {len(self.consecutive_accident_details)} frames."
        )
        ...
        for detail in self.consecutive_accident_details:
            if priority[detail["method"]] > priority[best_method]:
                ...
                priority[detail["method"]] == priority[best_method]
                and detail["value"] > best_value
            ):
                best_value = detail["value"]
    ...
    def _select_and_report_best_event(self):
        ...
        self.logger.info(
            f"Selecting the best event from {len(self.confirmed_accident_events)} confirmed accident(s)."
        )
    ...
    def _process_frame_results(self, results):
        ...
        for conf, cls_id in zip(confs, clss):
            if (
                self.model.names[int(cls_id)] == "vehicle_incident"
                and conf > self.acc_conf_thresh
            ):
    ...
    def _calculate_iou(self, box1, box2):
        ...
        if inter_area == 0:
            return 0.0
        box1_area, box2_area = (box1[2] - box1[0]) * (box1[3] - box1[1]), (
            box2[2] - box2[0]
        ) * (box2[3] - box2[1])
    ...
    def _annotate_frame(self, frame):
        ...
        if len(self.consecutive_accident_details) > 0:
            ...
            cv2.putText(
                frame, text, (50, 50), cv2.FONT_HERSHEY_SIMPLEX, 1, (0, 165, 255), 2
            )
        if self.accident_cooldown > 0:
            cv2.putText(
                frame,
    ...
    def _save_report(self, lat, lon, description_json):
        ...
        report = {
            ...
            "detection_parameters": {
                ...
                "speed_threshold_for_stop": self.speed_threshold,
...
def main():
    ...
    parser.add_argument(
        "--model", type=str, default="best.pt", help="Path to YOLO model."
    )
    parser.add_argument(
        "--output", type=str, default="results_video.mp4", help="Output video file."
    )
    parser.add_argument(
        "--acc-conf",
        type=float,
        default=0.75,
        help="Accident classification confidence threshold.",
    )
    parser.add_argument(
        "--other-conf",
        type=float,
        default=0.60,
        help="Other object confidence threshold.",
    )
    parser.add_argument(
        "--frame-threshold",
        type=int,
        default=3,
        help="Consecutive frames to confirm an accident.",
    )
    parser.add_argument(
        "--speed-threshold",
        type=int,
        default=10,
        help="Displacement in pixels over 5 frames to be a 'stop'.",
    )
    parser.add_argument(
        "--iou-threshold",
        type=float,
    ...
    parser.add_argument(
        ...
        type=str,
        default=None,
        help="Location name or address for accurate geocoding (e.g., 'Eiffel Tower').",
    )
    ...
    detector = AccidentDetector(
        ...
        output_video_filename=args.output,
        acc_conf_thresh=args.acc_conf,
        other_conf_thresh=args.other_conf,
        frame_confirmation_threshold=args.frame_threshold,
