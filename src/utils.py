...
def get_gps_coordinates(manual_coords=None, location=None):
    ...
    logging.warning(
        "No location provided or geocoding failed. Falling back to IP-based location, which can be inaccurate."
    )
    try:
        g = geocoder.ip("me")
        if g.ok:
            ...
            logging.warning(
                "Could not get location from IP. Returning default coordinates."
            )
...
def get_gemini_api_key():
    ...
    if not api_key:
        logging.warning(
            "GEMINI_API_KEY is not set. Accident descriptions will not be generated."
        )
        logging.warning(
